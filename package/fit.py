...
def fd(agent, data, env, method='mle',
    ...
    result = minimize(nll, param0, args=(agent, env, data), 
    ...
    fit_res['negloglike']   = nll(result.x, agent, env, data)
...
"fit parallel"
def fp(pool, agent, data, env, n_fits=40,             
...
def fl(pool,agent,datalist,env,n_fits):
    ...
    for dataname in datalist.keys():
        ...
        opt_res = fp(pool, agent, data, env, n_fits, method='mle', 
